...
// return json tokens instead of setting cookies
export async function verifyMobile(req: Request, res: Response) {
  try {
    ...
    return res.json({ token: signed }); // store securely
  } catch (err) {
...
export async function loginMobile(req: Request, res: Response) {
  try {
    ...
    return res.json({ token }); // store securely
  } catch (err) {
    console.error("loginMobile error:", err);
    return res.status(500).json({ error: "Server error" });
  }
}
...
// REGISTER FOR MOBILE:
// sends verify link that points to the mobile verify endpoint
export async function registerMobile(req: Request, res: Response) {
  try {
    const { email, username, password } = req.body;
    if (!email || !username || !password) {
      return res.status(400).json({ error: "All fields required" });
    ...
    // mobile verify endpoint (returns JSON token for the app)
    const verifyUrlMobile = `${BACKEND_URL}/auth/verify/mobile?token=${token}`;
    ...
    const msg = {
      ...
      html: `
        <p>Hi ${username},</p>
        <p>Click the link below to verify your account (mobile):</p>
        <a href="${verifyUrlMobile}">Verify account</a>
        <p>This link expires in 24 hours.</p>
      `,
    };
    ...
    return res.status(201).json({ message: "User registered. Check your email to verify the account (mobile link)." });
  } catch (error) {
    console.error("registerMobile error:", error);
    return res.status(500).json({ error: "Server error" });
  }
}
